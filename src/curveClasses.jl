...
"""
function GKM_second_homology(G::AbstractGKM_graph, cohomR::GKM_cohomology_ring)::GKM_H2
  ...
  for e in edges(G.g)
    edgeCNDict[edgeToGenIndex[e]] = chernNumber(e, cohomR) * gens(ZZasModule)[1]
  end
  for i in 1:nEdges
...
"""
function chernNumber(e::Edge, R::GKM_cohomology_ring)::ZZRingElem
...
"""
function chernNumber(H2::GKM_H2, beta::AbstractAlgebra.Generic.QuotientModuleElem{ZZRingElem}; check::Bool=true)::ZZRingElem
  if check
...
"""
function isStrictlyNEF(G::AbstractGKM_graph, R::GKM_cohomology_ring)::Bool
  for e in edges(G.g)
    if chernNumber(e, R) <= 0
      return false
    end
  end
  return true
...
function _effectiveClassesWithChernNumber(
  ...
  if check
    @req isStrictlyNEF(H2.gkm, R) "Space is not strictly nef, so there are infinitely many effective curve classes with bounded chern number."
  end
...
function all_classes(G::AbstractGKM_graph, R::GKM_cohomology_ring)
  H2 = GKM_second_homology(G, R);
